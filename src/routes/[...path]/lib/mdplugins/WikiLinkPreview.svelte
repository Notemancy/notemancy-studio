<script lang="ts">
	...
	async function getContent(mdpath: string): Promise<string> {
		const response = await invoke("get_page_content", { virtualPath: mdpath });
		if (typeof response !== "object" || response === null) {
			throw new Error("Unexpected response format");
		}
		// Assume response is an object with a `content` field.
		const { content } = response as { content: string; metadata: any };
		return content;
	}
